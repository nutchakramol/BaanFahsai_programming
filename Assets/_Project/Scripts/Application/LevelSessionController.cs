...
public class LevelSessionController : MonoBehaviour
{
    ...
    {
        _currentLevelController = new LevelController(_currentLevelData, allSchemas);

        Dictionary<string, ItemSchemaSO> schemaLookup = allSchemas.ToDictionary(s => s.itemId, s => s);
        if (paletteView != null)
            paletteView.BuildPalette(_currentLevelData, schemaLookup, _currentLevelController);

        HashSet<string> activeRoomIds = new HashSet<string>(_currentLevelData.rooms.Select(r => r.roomId));
        foreach (var roomView in roomViews)
        {
            if (roomView != null)
                roomView.gameObject.SetActive(activeRoomIds.Contains(roomView.RoomId));
        }
    ...
    private void HandleLevelChecked(int stars, float overallPercent, bool canProceed)
    {
        ...
        {
            LevelProgress.UnlockUpTo(_currentLevelData.levelIndex + 1);
        }
    }
}